"""
Script to extract the reference scale from JSON files generated by Phenopype and save it in a CSV file.
The script reads all JSON files in the specified input directory, extracts the reference scale from each file, and writes the specimen name and scale to a CSV file. 
The specimen name is derived from the filename by removing the .json extension and any trailing annotations.
"""
